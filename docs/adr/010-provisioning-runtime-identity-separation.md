...
A new IAM user `cerberus-admin`, created once via the root account, configured as an AWS
CLI named profile `cerberus-admin`. Used **only** for `tofu apply` and `tofu destroy`
runs. Holds broad provisioning permissions across S3, Glue, EMR Serverless, Athena, and
CloudWatch, plus scoped IAM-write permissions covering exactly two named ARNs (see below).
...
`cerberus-admin` is a plain IAM user with long-lived access keys stored in the local AWS
CLI credentials file, not an IAM role gated by MFA assumption.
...
The safety boundary that makes Terraform-managed IAM acceptable here: `cerberus-admin`'s
...
The actual policy JSON for these scoped IAM-write permissions lives in `infra/`; it is not
enumerated here.
...
- **cerberus-cli needs a broader runtime grant than ADR 008 sketched.** ADR 008 described
  ...
  on Cerberus buckets, and CloudWatch log reads. The `infra/` IAM module must implement
  this grant. The final policy JSON lives in code; this ADR records only the scope
  constraint: **operational access, never infrastructure-changing access.**
