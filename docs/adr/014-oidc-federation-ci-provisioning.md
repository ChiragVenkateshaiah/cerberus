...
load-bearing. A `sub` scoped too broadly (e.g. `repo:<owner>/cerberus-platform:*`) would let
any branch or any pull request assume the role. The conditions are tight:

- `cerberus-ci-apply` trusts only `repo:<owner>/cerberus-platform:ref:refs/heads/main`
  (and, once `prod` exists, a GitHub Environment condition gating it further).
- `cerberus-ci-plan` trusts `repo:<owner>/cerberus-platform:pull_request`.
